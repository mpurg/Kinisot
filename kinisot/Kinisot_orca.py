...
class calc_rpfr:
   ...
   def __init__(self, outfile, isomer, *args):
      ...
      if is_linear(outfile) == 'linear': trans_rot_modes = 5
...
if __name__ == "__main__":
   # Takes arguments: ORCA_output_files and optional temperature and vibrational scaling factor
   files = []
   # Default values
   ...
   # Check that 2 ORCA files were specific with an isotopic substitution
   if not len(files) == 2 or label == "none": log.Fatal("\nWrong number of arguments used. Correct format: Kinisot.py 2 x orca_output_files -iso 1 2 3 (-t <temp>) (-s <scalefactor>) (-cutoff <value>)\n")
